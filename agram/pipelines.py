...
class AgramPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('agram.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `agram`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM agram WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO agram (date, title, link, content)"
